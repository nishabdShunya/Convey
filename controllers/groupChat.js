...
exports.getGroupMembers = async (req, res, next) => {
  try {
    ...
    const notMembers = allUsers.filter(
      (user) => !members.filter((member) => member.id === user.id).length
    );
    res.status(200).json({
      success: true,
...
exports.postAddMsg = async (req, res, next) => {
  const currentGroup = await Group.findOne({
    where: { name: req.query.chatGroup },
  });
...
exports.getGroupMsgs = async (req, res, next) => {
  try {
    ...
    const groupMessages = await Message.findAll({
      where: { groupId: currentGroup.id, id: { [Op.gt]: lastMsgId } },
    });
...
exports.postMakeAdmin = async (req, res, next) => {
  try {
    const groupAndUser = await GroupAndUsers.findOne({
      where: { id: req.body.groupAndUsersId },
    });
    await groupAndUser.update({ isAdmin: true });
    const userMadeAdmin = await User.findOne({
      where: { id: groupAndUser.userId },
    });
    res
      .status(201)
      .json({ success: true, nameOfUserMadeAdmin: userMadeAdmin.name });
  } catch (error) {
    res
      .status(500)
      .json({
...
exports.removeMember = async (req, res, next) => {
  try {
    const groupAndUser = await GroupAndUsers.findOne({
      where: { id: req.params.groupAndUsersId },
    });
    const userBeingRemoved = await User.findOne({
      where: { id: groupAndUser.userId },
    });
    await GroupAndUsers.destroy({ where: { id: req.params.groupAndUsersId } });
    res
      .status(200)
      .json({ success: true, nameOfMemberRemoved: userBeingRemoved.name });
  } catch (error) {
    res
      .status(500)
      .json({
...
exports.postAddMember = async (req, res, next) => {
  try {
    const userAdded = await User.findOne({
      where: { email: req.body.addMemberInput },
    });
    const addedToGroup = await Group.findOne({
